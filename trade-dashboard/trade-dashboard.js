...
var BarChart, PieChart, applyTrade, fetchSalaries, fetchStats, fetchTrades, getURLParameter, initApp, initSalaries, initStats, initTeamDropdown, initTrades, load, playerNameMatcher, playersFromTeam, playersNotFromTeam, reRenderForTeam, renderPlayerTable, renderTradeDropdown, renderTrades, salaryString, sortPlayersBySalary, sortTeams, tradeUpdate, transformData;
...
fetchTrades = function() {
  return $.ajax({
    ...
    dataType: "jsonp",
    success: function(data) {
      return initTrades(data);
    }
  });
};

initTrades = function(data) {
  data.forEach(function(trade) {
    var tradedPlayer;
    tradedPlayer = _.find(window.statsData, function(player) {
      return player.name === trade.name;
    });
    if (tradedPlayer) {
      return tradedPlayer.team = trade.team;
    }
  });
  return initApp();
};
